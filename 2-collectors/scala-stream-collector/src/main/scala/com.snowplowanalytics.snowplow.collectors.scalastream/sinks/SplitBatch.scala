...
object SplitBatch {
   ...
   */
  def split(input: List[String], maximum: Long): SplitBatch = {
    ...
    def iterbatch(
      ...
      case h :: t => {
        ...
        } else if (headSize + currentTotal > maximum) {
          iterbatch(l, Nil, 0,  currentBatch :: acc, failedBigEvents)
        } else {
          iterbatch(t, h :: currentBatch, headSize + currentTotal, acc, failedBigEvents)
        }
      }
    }
